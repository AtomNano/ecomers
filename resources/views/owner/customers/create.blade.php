...
@section('title', 'Tambah Pelanggan - Owner')

@section('content')
<div class="max-w-2xl mx-auto">
    <h1 class="text-3xl font-bold mb-6">Tambah Pelanggan</h1>

    <form action="{{ route('owner.customers.store') }}" method="POST" class="bg-white rounded shadow p-6 space-y-4">
        @csrf
        ...
        <div>
            ...
            <textarea name="address" class="w-full border rounded px-3 py-2" rows="3" required>{{ old('address') }}</textarea>
            @error('address')
                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
